...
assign Ram_Addr= seq_en ? {25'b0,seq_addr,2'b00} : (switch_en ? (add_a<<2) : ALUResult_mem);
DataRamDl data_unit(.clk(clk),.addr(Ram_Addr),.din(MemWriteData_mem),.we(MemWrite_mem),.dout(RamOut_mem),.dduaddr(switch_addr<<2),.dduout(tmpdata));
DataRamDl data_unit(.clk(clk),.addr(`IO_REGA),.din(add_a),.we(1));
DataRamDl data_unit(.clk(clk),.addr(`IO_REGB),.din(add_b),.we(1));
